...
void tentarReconectarWiFi() {
  // --- MODIFICAÇÃO: SÓ TENTA CONECTAR SE SSID E SENHA EXISTIREM ---
...
void setup() {
  ...
  // --- MODIFICAÇÃO: CARREGA O CÓDIGO PRIMEIRO PARA USAR NO NOME BLE ---
  ...
  // --- MODIFICAÇÃO: SÓ TENTA CONECTAR SE AS CREDENCIAIS FORAM CARREGADAS ---
  if (ssid.length() > 0 && password.length() > 0) {
    WiFi.begin(ssid.c_str(), password.c_str());
  }
