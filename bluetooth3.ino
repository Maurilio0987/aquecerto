...
// ======================================================================
// CLASSE DE CALLBACK DA CARACTERÍSTICA BLE
// ======================================================================
class MyCallbacks: public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *pCharacteristic) {
    Serial.println("--- onWrite callback ACIONADO! ---");

    // Recebe o valor diretamente como uma string JSON
    std::string receivedJson = pCharacteristic->getValue();

    if (receivedJson.length() > 0) {
      Serial.print("Dado recebido (JSON): ");
      Serial.println(receivedJson.c_str());

      bool needsRestart = false;
      StaticJsonDocument<256> newConfigDoc;
      
      // Tenta desserializar a string JSON recebida
      DeserializationError error = deserializeJson(newConfigDoc, receivedJson);
...
// ======================================================================
// ✅ CLASSE DE CALLBACK DO SERVIDOR BLE - ADICIONADA PARA CORRIGIR A DESCONEXÃO
// ======================================================================
class MyServerCallbacks: public BLEServerCallbacks {
    void onConnect(BLEServer* pServer) {
      Serial.println("####################################");
      Serial.println("### Dispositivo Conectado (BLE)  ###");
      Serial.println("####################################");
    }

    void onDisconnect(BLEServer* pServer) {
      Serial.println("####################################");
      Serial.println("### Dispositivo Desconectado (BLE) ###");
      Serial.println("####################################");
      // Reinicia o advertising para permitir que outros dispositivos se conectem
...
// ... (O restante do seu código, como a função gerarCodigo, calcularBrilho, etc., permanece o mesmo) ...
...
void enviarDadosSupabase(float temperatura, int brilho, float umidade, int estadoRele) {
  ...
  if (httpCode > 0) {
    // Silenciado para não poluir o log
  } else {
    Serial.printf("[Supabase] Erro no update principal: %s\n", http.errorToString(httpCode).c_str());
  }
  http.end();
}
...
void tentarReconectarWiFi() {
  if (ssid.length() == 0) {
    if (wifiConectado) {
      Serial.println("SSID vazio. Desconectando WiFi.");
      WiFi.disconnect();
    }
    wifiConectado = false;
    return;
...
// ======================================================================
// FUNÇÃO SETUP - ATUALIZADA COM AS CORREÇÕES
// ======================================================================
void setup() {
  Serial.begin(115200);
  delay(1000);
  ...
  // --- INICIALIZA O BLUETOOTH COM AS CORREÇÕES ---
  BLEDevice::init(codigo.c_str());
  BLEServer *pServer = BLEDevice::createServer();

  // ✅ CORREÇÃO 1: Atribuindo o callback do servidor para gerenciar conexões
  ...
  // ✅ MELHORIA: Definindo permissões de escrita explicitamente
...
void loop() {
  if (restartRequest) {
    delay(100); // Um pequeno delay para garantir que tudo foi processado
    ESP.restart();
  }
  esp_task_wdt_reset();
  ...
  if (!isnan(temperatura)) {
    ...
    brilho = 0;
    // Garante que o LCD seja atualizado mesmo com erro no sensor
    if (millis() - tempoUltimaTela >= intervaloTela) {
      tempoUltimaTela = millis();
      atualizarLCD(NAN, NAN, NAN, 0);
    }
    Serial.println("Erro na leitura do DHT");
  }

  unsigned long tempoAtual = millis();
  if (wifiConectado && tempoAtual - ultimoTempoVerificacao >= intervaloVerificacao) {
    ultimoTempoVerificacao = tempoAtual;
    verificarMudancaDeDia();
    if(!isnan(temperatura) && !isnan(umidade)){
      enviarDadosSupabase(temperatura, brilho, umidade, estadoRele);
    }
    Serial.println("Verificação periódica executada.");
  }
